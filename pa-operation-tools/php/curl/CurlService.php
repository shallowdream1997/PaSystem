...
class CurlService
{
    ...
    public function gateway(): CurlService
    {
        ...
        // 网关鉴权 token 从本地配置文件读取(php/config/gateway_token.php,不入库,防凭据泄露)
        // 部署时需创建该文件;缺失时使用空 token
        $requestKey = "";
        $tokenFile = dirname(__FILE__) . "/../config/gateway_token.php";
        if (is_file($tokenFile)) {
            $tokens = require $tokenFile;
            if (is_array($tokens) && isset($tokens[$this->environment])) {
                $requestKey = $tokens[$this->environment];
            }
        }
