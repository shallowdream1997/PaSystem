...
//                        "debug" => true
    ...
    public function buildFixSql(){
        ...
        $fileName = PA_EXPORT_PATH . "/sp/";
        try {
            // 修复:_readCSV 返回数据行数组(list 解构二元组为历史遗留缺陷,contentList 恒为 null 导致逻辑静默不执行)
            $contentList = $excelUtils->_readCSV($fileName . "mawenfengkeyword.csv");
